...
import {
  ...
  getCommunicationResultsById,
  getTechnicalResultsById,
} from "~/app/actions/interview";
import { GetResumeData } from "~/app/actions/resume";

const Analysis: React.FC<{ resume: GetResumeData }> = ({ resume }) => {
  ...
  const communicationKey: string = "communication-" + resume.resume.resumeHash;
  const technicalKey: string = "techinical-" + resume.resume.resumeHash;
  const resumeHash = resume.resume.resumeHash;
  ...
  useEffect(() => {
    ...
    async function fetchResults() {
      ...
      const savedCommData: CommunicationResultsResponse | null = JSON.parse(
        localStorage.getItem(communicationKey)!,
      ) as CommunicationResultsResponse;
      const savedTechData: TechnicalResultsResponse | null = JSON.parse(
        localStorage.getItem(technicalKey)!,
      ) as TechnicalResultsResponse;

      if (savedCommData && savedTechData) {
        setCommResults(savedCommData);
        setTechResults(savedTechData);
        setIsLoading(false);
      }
      ...
      try {
        ...
        const [commResponse, techResponse] = await Promise.all([
          getCommunicationResultsById(key, resume.resume.owner.id),
          getTechnicalResultsById(key, resume.resume.owner.id),
        ]);
        ...
        if (commResponse.success && techResponse.success) {
          const commData = JSON.stringify(commResponse.data);
          const techData = JSON.stringify(techResponse.data);

          localStorage.setItem(communicationKey, commData);
          localStorage.setItem(technicalKey, techData);
        }
      } catch (err) {
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="rounded-lg border border-red-200 bg-red-50 p-6 text-center">
          ...
          <p className="mb-4 text-red-600">
            We couldn&apos;t load the applicant&apos;s interview results. This might
            be because the interview is not yet complete or there was an error
      ...
      <div className="mb-8 flex flex-col items-start justify-between md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-2 text-[#6b5d4c]">
            Review the applicant&apos;s interview performance and feedback from our
